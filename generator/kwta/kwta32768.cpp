...
#define TRUNK 64 //number of branches
#define TRUNK_BIAS 6 //log2(TRUNK)
#define L 16 //number of branches in one HBM stack assuming there are 16 HBM ports
#define B 4  //number of banks depending on HBM
#define HEAP_BIAS 3 //number of log2(spaces in one mini heap) + 1
#define DATA_BIAS 10 //the log2(size of one space)
...
static ap_uint<2 * HEAP_BIAS> used_free[8192]; //one for free and one for allocate
    ...
    section0:{
        ...
        if (cmd == 2)
        {
            ...
            if(top_heap==0){
                ...
                {
					last_loc1[used_id] = loc1 = log_2_64bit((ap_uint<64>)(top_heap(used_id * L + L - 1, used_id * L)-(top_heap(used_id * L + L - 1, used_id * L)&(top_heap(used_id * L + L - 1, used_id * L)-1)))) + used_id * L; //TODO
