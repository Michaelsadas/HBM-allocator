...
#define BRANCH 128 //mini heap in one branch
#define BRANCH_BIAS 7 //log2(BRANCH)
#define TRUNK 128 //number of branches
#define TRUNK_BIAS 7 //log2(TRUNK)
#define L 32 //number of branches in one HBM stack assuming there are 16 HBM ports
#define B 4  //number of banks depending on HBM
#define HEAP_BIAS 3 //number of log2(spaces in one mini heap) + 1
#define DATA_BIAS 9 //the log2(size of one space)
...
static ap_uint<2 * HEAP_BIAS> used_free[16384]; //one for free and one for allocate
    ...
    section0:{
        ...
        if (cmd == 2)
        {
            ...
            if(top_heap==0){
                ...
                {
					last_loc1[used_id] = loc1 = log_2_256bit((ap_uint<256>)(top_heap(used_id * L + L - 1, used_id * L)-(top_heap(used_id * L + L - 1, used_id * L)&(top_heap(used_id * L + L - 1, used_id * L)-1)))) + used_id * L; //TODO
					last_loc2[used_id] = loc2 = log_2_256bit((ap_uint<256>)(heap_tree[loc1]-(heap_tree[loc1]&(heap_tree[loc1]-1)))); //TODO
