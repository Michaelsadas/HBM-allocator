...
#define BRANCH 16 //mini heap in one branch
#define BRANCH_BIAS 4 //log2(BRANCH)
#define TRUNK 8 //number of branches
#define TRUNK_BIAS 3 //log2(TRUNK)
#define L 2 //number of branches in one HBM stack assuming there are 16 HBM ports
#define B 4  //number of banks depending on HBM
#define HEAP_BIAS 3 //number of log2(spaces in one mini heap) + 1
#define DATA_BIAS 16 //the log2(size of one space)
...
static ap_uint<BRANCH> heap_tree[TRUNK]={-1, -1, -1, -1, -1, -1, -1, -1, };

static ap_uint<TRUNK> top_heap=-1;
static int last_offset[B]={-1, -1, -1, -1}; //20 should be a parameter
static ap_uint<2 * HEAP_BIAS> used_free[128]; //one for free and one for allocate
        ...
        if (cmd == 2)
        {
            ...
            if(top_heap==0){
                ...
                else
                {
					...
					last_loc2[used_id] = loc2 = log_2_16bit((ap_uint<16>)(heap_tree[loc1]-(heap_tree[loc1]&(heap_tree[loc1]-1)))); //TODO
